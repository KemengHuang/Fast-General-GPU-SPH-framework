                             ...
                             int num_blocks, int num_warps, int num_sms)
{
    ...
    //if (idx == 0)
    //{
    //    //printf("idx: %d, t_comp: %f, t_mem: %f, t_op: %f\n", idx, t_comp, t_mem, t_overlap);
    //    //printf("avg_inst_lat: %f, dram_lat: %f\n", avg_inst_lat, dram_lat);
...
void allocateGPUModel(GPUModel *&gpu_model)
{
    ...
    CUDA_SAFE_CALL(cudaMalloc(&(gpu_model->bs_tra_density), data_len * sizeof(PTXBlockStatistic)));
    CUDA_SAFE_CALL(cudaMemcpy(gpu_model->bs_tra_density, bs_tra_density, data_len * sizeof(PTXBlockStatistic), cudaMemcpyHostToDevice));
    data_len = readPTXStatisticsFromFile(bs_tra_force, kFunNameForceTRA, kPTXStatisticsFileNameTRA);
    CUDA_SAFE_CALL(cudaMalloc(&(gpu_model->bs_tra_force), data_len * sizeof(PTXBlockStatistic)));
    CUDA_SAFE_CALL(cudaMemcpy(gpu_model->bs_tra_force, bs_tra_force, data_len * sizeof(PTXBlockStatistic), cudaMemcpyHostToDevice));
    data_len = readPTXStatisticsFromFile(bs_sms_density, kFunNameDensitySMS, kPTXStatisticsFileNameSMS);
    CUDA_SAFE_CALL(cudaMalloc(&(gpu_model->bs_sms_density), data_len * sizeof(PTXBlockStatistic)));
    CUDA_SAFE_CALL(cudaMemcpy(gpu_model->bs_sms_density, bs_sms_density, data_len * sizeof(PTXBlockStatistic), cudaMemcpyHostToDevice));
    data_len = readPTXStatisticsFromFile(bs_sms_force, kFunNameForceSMS, kPTXStatisticsFileNameSMS);
    CUDA_SAFE_CALL(cudaMalloc(&(gpu_model->bs_sms_force), data_len * sizeof(PTXBlockStatistic)));
    CUDA_SAFE_CALL(cudaMemcpy(gpu_model->bs_sms_force, bs_sms_force, data_len * sizeof(PTXBlockStatistic), cudaMemcpyHostToDevice));
    ...
    CUDA_SAFE_CALL(cudaMalloc(&(gpu_model->static_block), 4 * sizeof(PTXBlockStatistic)));
    CUDA_SAFE_CALL(cudaMemcpy(gpu_model->static_block, static_block, 4 * sizeof(PTXBlockStatistic), cudaMemcpyHostToDevice));

    // read instruction latency and transfer to device
    readInstructionLatencyFromFile(insts_latency, kLatencyFileName);
    CUDA_SAFE_CALL(cudaMemcpyToSymbol(kDevInstsLatency, &insts_latency, sizeof(InstructionInfo)));
    ...
    device_info.ilp = 2;
    device_info.mlp = 2;
    CUDA_SAFE_CALL(cudaMemcpyToSymbol(kDevGPUInfo, &device_info, sizeof(GPUDeviceInfo)));
    ...
    CUDA_SAFE_CALL(cudaMalloc(&(gpu_model->kn_paras), 4 * sizeof(KernelRelatedParas)));
    CUDA_SAFE_CALL(cudaMemcpy(gpu_model->kn_paras, kn_paras, 4 * sizeof(KernelRelatedParas), cudaMemcpyHostToDevice));
...
{
    CUDA_SAFE_CALL(cudaFree(gpu_model->bs_tra_density));
    CUDA_SAFE_CALL(cudaFree(gpu_model->bs_tra_force));
    CUDA_SAFE_CALL(cudaFree(gpu_model->bs_sms_density));
    CUDA_SAFE_CALL(cudaFree(gpu_model->bs_sms_force));
    CUDA_SAFE_CALL(cudaFree(gpu_model->kn_paras));
    CUDA_SAFE_CALL(cudaFree(gpu_model->static_block));
